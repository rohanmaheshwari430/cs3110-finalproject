...
(*
type json_type = {
  students: People.student list;
  professors: People.professor list;
  courses: Courses.t
...
let p_c_json j = {
  students = j |> member "people" |> to_list;
  professors = j |> member "professors" |> to_list;
  courses = j |> member "courses" |> to_list |> List.map course_of_json;
...
let menu n = 
  ...
  while not !quit_loop do 
    ...
    if (option = 1) then 
      ...
      (print_string "Enter an Assignment ID: "; let id = read_int () in
      print_newline ();
      print_string "Enter an Assignment title: "; let title = read_line () in
      print_newline ();
      print_string "Enter the weight of this assignment: "; let weight = read_int () in 
      print_newline (); 
      print_string "Enter the course id for this assignment: "; let course_id = read_int () in
      ...
      (print_string "Enter an Assignment ID: "; let id = read_int () in
      print_newline ();
      print_string "Enter the field to edit (title, id, weight): "; let field = read_line () in
      print_newline ();
      print_string "Enter the course id for this assignment: "; let course_id = read_int () in 
      if field = "title" then 
        (print_string "Enter a new title: "; let title = read_line () in 
        Courses.edit_assignment_in_course cms_courses id field title course_id)
      else if field = "id" then 
        (print_string "Enter a new id: "; let new_id = read_int () in
        Courses.edit_assignment_in_course cms_courses id field (string_of_int new_id) course_id)
      else if field = "weight" then 
        (print_string "Enter a new weight: "; let new_weight = read_int () in
        Courses.edit_assignment_in_course cms_courses id field (string_of_int new_weight) course_id)
        )
      ...
      (print_string "Enter the netid of the professor: "; let netid = read_line () in
      print_newline ();
      print_string "Enter the id of the course: "; let course_id = read_int () in
      print_newline();
