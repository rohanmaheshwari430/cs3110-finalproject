...
type assignment = {
  ...
  id: int;
  weight: int;
(* student_grades: int * int list *) (*To be implemented in MS2*)
}
...
type student = {
  ...
  netid: int;
  grad_year: int;
  courses: string * int list; (* (course title, course id) *)
}
...
(** [find_courses] will find a course by id [i]. 
    Returns an optional course 
*)
let find_course i c = 
  try Some (c |> List.filter (fun x -> x.id = i) |> List.hd) with
  Failure hd -> None

(** [pp_student] will pretty print the high-level
    fields of a student
*)
...
let delete_course i c = 
  c := List.filter (fun x -> x.id <> i) !c
